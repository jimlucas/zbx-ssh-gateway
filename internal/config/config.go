...
type Config struct { Server ServerConfig `yaml:"server"`; API APIConfig `yaml:"api"`; SSH SSHConfig `yaml:"ssh"`; Limits LimitsConfig `yaml:"limits"`; Operations map[string]Operation `yaml:"operations"` }
...
func Load(path string)(*Config,error){b,e:=os.ReadFile(path);if e!=nil{return nil,e};var c Config;if e=yaml.Unmarshal(b,&c);e!=nil{return nil,e};c.SSH.ConnectTimeout=time.Duration(c.SSH.ConnectTimeoutSeconds)*time.Second;c.SSH.CommandTimeout=time.Duration(c.SSH.CommandTimeoutSeconds)*time.Second;if e=c.Validate();e!=nil{return nil,e};return &c,nil}
